 ...
 (block run-tests
   (let* ((suite-name (getf args :suite))
     ...
     (unless fiveam-pkg (error "FiveAM package not found. Load FiveAM first."))
      (let* ((run-sym (find-symbol "RUN" fiveam-pkg))
             (run-fn (and run-sym (fboundp run-sym) (symbol-function run-sym)))
             (test-dribble-sym (find-symbol "*TEST-DRIBBLE*" fiveam-pkg)))
        (unless run-fn (error "FiveAM:RUN function not found"))
        (let ((*standard-output* (make-string-output-stream))
              (*error-output* (make-string-output-stream)))
          (flet ((%collect-result (results)
                   (multiple-value-bind (total passed failed failures)
                     ...
                     (let ((result (make-hash-table :test 'equal)))
                       ...
                       result))))
            (if test-dribble-sym
                (progv (list test-dribble-sym)
                       (list (make-broadcast-stream))
                  (%collect-result (funcall run-fn target)))
                (%collect-result (funcall run-fn target)))))))))
